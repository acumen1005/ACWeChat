...
@interface RoomChatViewController ()<UITableViewDataSource,UITableViewDelegate,XMPPStreamDelegate,XMPPRoomStorage,XMPPRosterDelegate,XMPPRoomDelegate,NSFetchedResultsControllerDelegate>
...
- (NSFetchedResultsController *)fetchedResultsController {
    ...
    // 每一个聊天界面，只关心聊天对象的消息
    request.predicate = [NSPredicate predicateWithFormat:@"roomJIDStr = %@", self.jid.full];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    // 判断是发出消息还是接收消息
    NSString *ID = ([message.fromMe intValue] != 1) ? @"MyChatCell" : @"OtherChatCell" ;
    if([ID isEqualToString:@"MyChatCell"]){
        ...
        if ([message.message saveAttachmentJID:self.jid.bare timestamp:message.localTimestamp]) {
            message.messageStr = [message.message compactXMLString];
            
            [[XMPPTool sharedXMPPTool].xmppRoomCoreDataStorage.mainThreadManagedObjectContext save:NULL];
        }
        
        NSString *path = [message.message pathForAttachment:self.jid.bare timestamp:message.localTimestamp];
...
- (void) actionToSend{
    [[XMPPTool sharedXMPPTool].xmppRoom sendMessageWithBody:_textField.text];
